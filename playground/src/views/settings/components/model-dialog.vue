...
<template>
  <div v-if="dialogVisible" class="fixed inset-0 z-20 grid place-items-center p-5 bg-mc-ink-950/32 backdrop-blur-[5px]" @click.self="close">
    <div class="w-dialog-sm p-6.25 bg-mc-paper-50 border border-mc-paper-0/82 rounded-3xl shadow-dialog" role="dialog" aria-modal="true" aria-labelledby="model-dialog-title">
      ...
      <form class="dialog-form flex flex-col gap-3.5" @submit.prevent="handleSubmit">
        <label class="field flex flex-col gap-1.5 text-sm font-[750] text-mc-ink-950">
          <span>模型名称</span>
          <input v-model="formModel.model_name" class="w-full min-h-9.5 px-2.75 text-base text-mc-ink-950 outline-none bg-mc-paper-0 border border-mc-sage-600/18 rounded-md focus:border-mc-sage-600 focus:shadow-focus" required placeholder="例如 gpt-4o" autocomplete="off">
        </label>
