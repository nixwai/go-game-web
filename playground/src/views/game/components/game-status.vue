...
<template>
  <div class="flex flex-col gap-2.25">
    <div class="flex gap-2.5 items-center p-3 bg-mc-paper-50 border border-mc-neutral-200/80 rounded-xl">
      <div
        class="w-7.25 h-7.25 bg-mc-stone-950 border border-mc-stone-950 rounded-full shadow-[1px_2px_4px] shadow-mc-stone-950/23 dark:!bg-[#000] dark:!border-[#cacbc4] dark:shadow-[#cacbc4]/23"
        :class="{ '!bg-mc-paper-0 !border-mc-stone-200 dark:!bg-[#fff] dark:!border-[#cacbc4]': !playerIsBlack }"
        aria-hidden="true"
      />
    ...
    <div class="grid grid-cols-[1fr] gap-2.25">
    ...
    <div
      class="flex gap-1.75 items-center px-2.5 py-2.25 text-sm font-[700] text-mc-sage-680 bg-mc-sage-100 rounded-md"
      :class="{
    ...
    >
      <i
        class="w-1.5 h-1.5 bg-mc-sage-450 rounded-full"
        :class="{ 'flex-none mt-1 !bg-mc-danger-600': Boolean(props.aiError), '!bg-mc-ochre-450': props.isAIThinking && !props.aiError }"
      />
      <span class="flex-1">
      ...
      <button
        v-if="props.aiError"
        class="flex-none px-1.5 py-0.5 text-xs font-[750] text-mc-danger-600 cursor-pointer bg-mc-transparent border border-mc-danger-600 rounded-xs hover:text-mc-paper-0 hover:bg-mc-danger-600"
        type="button"
        @click="$emit('retry')"
      >
        重试
